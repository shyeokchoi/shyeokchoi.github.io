...
이 코드가 일반적으로 행렬곱을 구하는 것보다 빠른 이유는 \\(A\\), \\(C\\)에 접근할 때 항상 연속된 주소를 참조하여 `spatial locality`를 잘 활용하기 때문입니다. \\(B\\)에 접근할 때도 만약 `block size`가 `block` 전체를 `Cache`에 올릴 수 있을만큼 충분히 작다면 `Cache`의 빠른 접근속도를 누릴 수 있습니다.  
