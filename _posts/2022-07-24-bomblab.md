...
예를 들어, \\(d_i\\)가 `1`이라면, `Mem[%rsp + 0x20]`에 `0x6032d0`가 저장됩니다.  
...
문제는 `Node`들을 어떻게 연결할 것인가 인데, 이는 `<+241>, <+243>`을 보면 알 수 있습니다. 즉, 앞선 `Node`의 값이 뒤따르는 `Node`의 값보다 커야 합니다. 내림차순으로 정렬하면 되겠습니다.  
...
지금까지의 과정에 따라, `3 4 5 6 1 2`를 입력해야 함을 알 수 있습니다.  
단, 앞에서 \\(7 - d_i\\)의 과정을 거쳤기 때문에 최종적으로 입력해야 하는 답은 `4 3 2 1 6 5`가 될 것입니다.  
